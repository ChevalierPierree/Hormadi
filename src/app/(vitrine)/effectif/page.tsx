...
function PlayerCard({ player }: { player: Player }) {
  return (
    <div className="bg-hormadi-surface border border-hormadi-border rounded-xl overflow-hidden hover:border-white/20 transition-all duration-300 group">
      <div className="aspect-[3/4] bg-hormadi-dark relative overflow-hidden">
        {player.photoUrl ? (
          <img
            src={player.photoUrl}
            alt={player.name}
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-hormadi-forest to-hormadi-dark">
        ...
        {player.number !== undefined && player.number !== null && (
          <div className="absolute top-2 left-2 bg-hormadi-red text-white font-black text-sm w-8 h-8 rounded-full flex items-center justify-center">
            {player.number}
          </div>
        )}
      </div>
      <div className="p-3">
        ...
        {player.nationality && <p className="text-hormadi-muted text-xs mt-1">{player.nationality}</p>}
      </div>
    </div>
  )
}
